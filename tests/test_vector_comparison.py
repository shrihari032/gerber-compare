...
from gerber_xor import GerberError, Parser, compare
...
"""
    ...
    def test_unknown_aperture_is_parsed_then_fails_at_geometry_resolution(self):
        unknown = MM.replace("%ADD10C,1.000000*%", "%ADD99TOTALLY_UNKNOWN_SHAPE,0.1X0.2X0.3*%").replace("D10*", "D99*")
        with self.assertRaisesRegex(GerberError, "Unsupported aperture geometry"):
            self.parse(unknown)
